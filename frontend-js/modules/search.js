...
export default class Search {
  ...
  keyPressHandler() {
    ...
    if (value != '' && value != this.previousValue && value.length > 3) {
      clearTimeout(this.typingWaitTimer);
      this.showLoaderIcon();
      this.hideResultsArea();
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      this.resultArea.innerHTML = DOMPurify.sanitize('<p class="alert alert-danger text-centered shadow-sm">Sorry, we could not find any results for that search</p>');
    }
  ...
  openOverlay() {
    ...
    // wait a little bit for input to show and then focus on it
